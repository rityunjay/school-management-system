...
class Member extends CI_Model{
    ...
    public function update_user_detail()
    {
        ...
        $userfile = $_FILE['userfile']['name'];
        if ($userfile != '') {
            $photoExt1 = @end(explode('.',$userfile));
            $phototest1 = strtolower($photoExt1);
            $new_photo = time().'.'.$phototest1;
            $photo_path = './assets/images/faces/'.$new_photo;
            move_uploaded_file($_FILE['userfile']['tmp_name'],$photo_path);
        }
        if ($userfile=='') {
            $new_photo = $old_photo;
        }
